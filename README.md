...
>
>我们每一个营业网点都可以画一个作业范围在地图上，画好这个范围组成一个闭合的多边形，这个多边形是由多个坐标点组成的，然后利用 MongDB 把多个坐标点存储到数据库中，在 MongDB 中提供了根据多个作业范围内查询的一个方法（intercpet）,查询的时候参数只需要传递一个坐标点，返回的就是和这个坐标点有交差的多边形对应的数据查询出来，这个多边形关联的就是我们的机构网点ID
